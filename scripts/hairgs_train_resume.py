...
def training(
    ...
    if resume_step < 0 or resume_step >= opt.iterations:
        raise ValueError(
            f"resume_step must be in [0, {opt.iterations}), got {resume_step}"
        )
    if resume_step and not scene.loaded_iter:
        raise ValueError("resume_step requires an existing point-cloud iteration")
    ...
    progress_bar = tqdm(
        range(resume_step, opt.iterations + 1),
        desc="Training progress",
    )

    # Training loop
    for iteration in range(resume_step + 1, opt.iterations + 1):
        resumed_iteration = iteration - resume_step
        training_info.iter = scene.loaded_iter + resumed_iteration
        ...
        with torch.no_grad():
            ...
            if iteration % args.save_frequency == 0 or iteration == opt.iterations:
                ...
                scene.save(resumed_iteration)

    print(
        f"Training completed, gaussians saved to {scene.model_path}/point_cloud/iteration_{scene.loaded_iter + opt.iterations - resume_step}/point_cloud.ply"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Completed local optimization steps. Continue the original LR/densification schedule from this step.",
    )
