...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sample",
        default=None,
        choices=["wStraight", "wCurly", "wWavy", "wWavyThin"],
        help="Parse only one hairstyle instead of all four.",
    )
    parser.add_argument(
        "--download-root",
        required=True,
        help="Directory containing the downloaded Cem-Yuksel archives/assets.",
    )
    parser.add_argument(
        "--output-root",
        required=True,
        help="Separate output root; existing reduced-resolution data is never overwritten.",
    )
    args = parser.parse_args()
    DOWNLOAD_PATH = os.path.abspath(args.download_root)
    PARSED_PATH = os.path.abspath(args.output_root)
    if args.hsv:
    ...
    hairstyle_urls = [
        url
        for url in HAIRSTYLES_URL
        if args.sample is None
        or os.path.basename(url).replace(".zip", "") == args.sample
    ]
    for hairstyle_url in tqdm(hairstyle_urls):
