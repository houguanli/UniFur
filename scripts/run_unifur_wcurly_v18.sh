...
CONFIG="${UNIFIED_CONFIG:-${PROJECT_ROOT}/configs/fiber_hairgs_wcurly_scalproot_sh_v18_8k.yaml}"
RESIDUAL_CHECKPOINT="${RESIDUAL_CHECKPOINT:-${RESULT_ROOT}/cleanhair_v11_residual_teacher_4k_v1/unified_fiber_field.pt}"
OUT="${UNIFIED_OUT:-${RESULT_ROOT}/cleanhair_v18_scalproot_routesh_topology_8k_v1}"
...
evaluate_split() {
  ...
  "${CONDA_BIN}" run --no-capture-output -n dpd3dgs-animal python \
    ...
    --method "UniFur v18 scalp-root route-SH topology (${route})" \
