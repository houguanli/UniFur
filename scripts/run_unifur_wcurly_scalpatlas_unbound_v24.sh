...
CONFIG="${UNIFIED_CONFIG:-${PROJECT_ROOT}/configs/fiber_hairgs_wcurly_scalpatlas_unbound_v24_8k.yaml}"
RESIDUAL_CHECKPOINT="${RESIDUAL_CHECKPOINT:-${RESULT_ROOT}/cleanhair_v11_residual_teacher_4k_v1/unified_fiber_field.pt}"
OUT="${UNIFIED_OUT:-${RESULT_ROOT}/cleanhair_v24_scalpatlas_unbound_8k_v1}"
...
evaluate_split() {
  ...
  "${CONDA_BIN}" run --no-capture-output -n dpd3dgs-animal python \
    ...
    --method "UniFur v24 independent scalp atlas + unbound root birth (${route})" \
