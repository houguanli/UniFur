...
CONFIG="${UNIFIED_CONFIG:-${PROJECT_ROOT}/configs/fiber_hairgs_wcurly_cleanhair_unified_semantic_v12_6k.yaml}"
RESIDUAL_CHECKPOINT="${RESIDUAL_CHECKPOINT:-${RESULT_ROOT}/cleanhair_v11_residual_teacher_4k_v1/unified_fiber_field.pt}"
OUT="${UNIFIED_OUT:-${RESULT_ROOT}/cleanhair_v12_semantic_capacity_6k_v1}"
...
evaluate_split() {
  ...
  "${CONDA_BIN}" run --no-capture-output -n dpd3dgs-animal python \
    ...
    --method "UniFur v12 hard semantic capacity (${route})" \
