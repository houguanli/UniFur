...
REPRESENTATION="${1:-residual_only}"
...
case "${REPRESENTATION}" in
  residual_only)
    CONFIG="${PROJECT_ROOT}/configs/fiber_cat_residual_only.yaml"
    RUN_NAME="${RUN_NAME:-cat_residual_only_20k_observed40}"
    ROUTE_MODES=(residual)
    ;;
  unified)
    CONFIG="${PROJECT_ROOT}/configs/fiber_cat_full.yaml"
    RUN_NAME="${RUN_NAME:-cat_unified_20k_observed40}"
    ROUTE_MODES=(soft hard)
    ;;
  *)
    echo "Usage: $0 {residual_only|unified}" >&2
    exit 2
    ;;
esac
...
for route_mode in "${ROUTE_MODES[@]}"; do
  "${PYTHON_BIN}" -m dpd3dgs_animal.cli \
    ...
    --frame-dir "${DATA_ROOT}/frames" \
    --out-dir "${OUTPUT_ROOT}/observed40_${route_mode}" \
    --max-frames 40 \
    --frame-start 0 \
    --renderer hairgs \
