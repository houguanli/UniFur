...
CONFIG="${UNIFIED_CONFIG:-${PROJECT_ROOT}/configs/fiber_panda_multiview_routesh_v18.yaml}"
...
OUT="${UNIFIED_OUT:-${ROOT}/full_unified_routesh_v18_12k}"
...
evaluate_split() {
  ...
  "${CONDA_BIN}" run --no-capture-output -n dpd3dgs-animal python \
    ...
    --method "UniFur v18 route-specific SH (${route})" \
