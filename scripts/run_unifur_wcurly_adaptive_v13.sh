...
CONFIG="${UNIFIED_CONFIG:-${PROJECT_ROOT}/configs/fiber_hairgs_wcurly_cleanhair_adaptive_v13_6k.yaml}"
RESIDUAL_CHECKPOINT="${RESIDUAL_CHECKPOINT:-${RESULT_ROOT}/cleanhair_v11_residual_teacher_4k_v1/unified_fiber_field.pt}"
OUT="${UNIFIED_OUT:-${RESULT_ROOT}/cleanhair_v13_adaptive_residualfree_6k_v1}"
...
evaluate_split() {
  ...
  "${CONDA_BIN}" run --no-capture-output -n dpd3dgs-animal python \
    ...
    --method "UniFur v13 adaptive residual-free (${route})" \
